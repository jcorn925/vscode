...
4. **Verifier** — deterministic checks (manifest fields, subsystem paths, scaffold files) plus optional Ix discovery match.
...
4. **Verifier**
   ...
   - Incomplete fixture returns structured gaps (`missing_path`, `missing_manifest_field`, `ix_no_match`).
...
- `5` — Shippable plan-verify loop with tests and commands
