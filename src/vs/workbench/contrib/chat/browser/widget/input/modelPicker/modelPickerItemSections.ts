...
export function buildUnavailableStateItems(options: IBuildModelPickerItemsOptions): IActionListItem<IActionWidgetDropdownAction>[] | undefined {
	...
	hover?.appendMarkdown(localize('chat.modelPicker.upgradeHover', "[Upgrade to {0} Pro](command:workbench.action.chat.upgradePlan \" \") to use the best models.", getDefaultChatProviderName()));
	return [{
		item: {
			id: 'noModels',
			enabled: false,
