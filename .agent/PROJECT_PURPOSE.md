...
Babadaba is a custom fork of Code - OSS focused on turning VS Code into a goal-command center for building, launching, and iterating on multi-surface products with agent assistance.

Babadaba keeps the strengths of VS Code as an editor and workbench, then adds opinionated product-building workflows on top: goal workspace manifests, startup readiness checks, app launch guidance, Ix CLI integration, custom mode surfaces, and a custom AI agent that understands cross-app planning and repo-local context.
...
Babadaba treats the user's goal as a first-class workspace object. The workbench should help the user understand what exists, what is ready, what needs to be launched, and how a change in one surface affects the others.

## Intended User Experience

Babadaba should feel like VS Code for people building an outcome rather than only editing a repository. Users should be able to:
