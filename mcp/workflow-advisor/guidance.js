...
/**
 * Curated, accurate descriptions of what GoalConsole actually does. This is the
...
const CAPABILITIES = {
	...
	previewWiring: 'Live preview wiring: each surface declares its dev command and local URL; GoalConsole probes the dev server and embeds the running app next to the plan, so generated code is verified visually as it lands.',
...
export function buildCapabilityMap(facts) {
	...
	switch (facts.classification) {
		case 'goal-workspace':
			add('taskTrees', 'high', `This is already a GoalConsole workspace (surfaces: ${facts.goalWorkspace.surfaces.join(', ') || 'none yet'}); the assessment should focus on unused capabilities.`);
			break;
		case 'empty-or-early':
			add('goalDecomposition', 'high', `The repo has ${facts.scale.sourceFiles} source files — greenfield is GoalConsole's strongest phase: state the goal, get surfaces proposed.`);
		...
		case 'multi-app-monorepo':
			add('goalDecomposition', 'high', `The workspace already has multiple apps (${names(appPackages)}); these map one-to-one onto GoalConsole surfaces, making the existing structure explicit and agent-legible.`);
			...
			break;
		case 'single-app':
			add('goalDecomposition', 'medium', `A single app (${names(appPackages) || facts.packages[0]?.dir}) often hides multiple surfaces (marketing, product, admin); GoalConsole would propose that decomposition when the next surface is added.`);
	...
	if (devServers.length) {
		add('previewWiring', 'high', `Detected dev scripts in ${names(devServers)} — GoalConsole would probe and embed these next to each surface's plan.`);
	}
	...
	if (!facts.deploy.vercel && !facts.deploy.netlify && !facts.deploy.githubActions) {
		...
		add('publishActions', 'low', 'Deployment is already wired up; GoalConsole publish actions would mostly add per-surface convenience.');
	}
...
/** Honest limits — where GoalConsole would help less. Credibility is the point. */
export function buildCaveats(facts) {
	const caveats = [];
	if (facts.scale.sourceFiles > 3000 || facts.scale.capped) {
		caveats.push('This is a large, established codebase. GoalConsole is strongest for the zero-to-one phase and for making new agent-built work legible; it will not restructure a mature repo, and day-to-day editing here would feel like VS Code plus goal tooling, not a transformation.');
	}
	...
	if (facts.classification === 'goal-workspace') {
		caveats.push('The repo already uses GoalConsole conventions — frame the answer as "capabilities you may not be using yet", not as an introduction.');
	}
	return caveats;
}
...
export const ASSISTANT_GUIDANCE = [
	...
	'Close with the one concrete first step: open this repo (or a new workspace) in GoalConsole and state the goal — the IDE proposes the surface breakdown from there.',
].join('\n- ');
